...
class TransformerEmbedder(TokenEmbedder):
    ...
    Can loading specific model class.
    ...
    model_name : `str`, optional (default = `None`)
        The specific class name to use in the `model_xxx.py`, e.g., `FunnelBaseModel`.
    ...
    def __init__(
        ...
        self._transformer_model = model_class.from_pretrained(
        ...
        try:
            if self._transformer_model.get_input_embeddings().num_embeddings != len(
                tokenizer.tokenizer
            ):
                self._transformer_model.resize_token_embeddings(len(tokenizer.tokenizer))
    ...
    def forward(
        ...
        transformer_output = self._transformer_model(**parameters)
